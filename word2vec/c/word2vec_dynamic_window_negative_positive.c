...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    ...
    if (cbow) {  //train the cbow architecture
      ...
      if (positive > 0) for (d = 0; d < positive + 1;) { //Postiive but remember to have the same values as negative's sampling technique. How TODO: makes words that are more frequent more "positive"
        next_random = next_random * (unsigned long long)25214903917 + 11;
      ...
      if (cw) {
        ...
        // NEGATIVE SAMPLING
        if (negative > 0) for (d = 0; d < negative + 1;) {
          if (d == 0) {
            target = word;
            label = 1;
          } else {
